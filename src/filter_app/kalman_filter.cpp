...
void kalman_filter::init_targets(vector<pair<int, vect3f>> positions,
        std::vector<sensor_parameters_dto> sensor_parameters) {
    for (auto it = sensor_parameters.begin(); it != sensor_parameters.end();
            ++it) {
        ublas::matrix<float> tmp = ublas::matrix<float>(6, 6, 0);
        float process_noise = it->process_noise_;
        tmp(0, 0) = tmp(1, 1) = tmp(2, 2) = (PERIOD_ * PERIOD_ / 2)
                * (PERIOD_ * PERIOD_ / 2);
        ...
        ublas::matrix<float> tmp2 = measurement_noise * measurement_noise
                * ublas::identity_matrix<float>(3);

        sensors_.insert(
                pair<int, shared_ptr<sensor_observer>>(it->id_,
                        shared_ptr<sensor_observer>(
                                new sensor_observer(it->id_, tmp2, tmp))));
    }
    for (auto it2 = positions.begin(); it2 != positions.end(); ++it2) {
        ...
        targets_.push_back(
                shared_ptr<target>(
                        new target(it2->first, tmp,
                                sensors_[it2->first]->get_process_covariance())));
    }
...
void kalman_filter::compute(vector<vect3f> new_positions) {
    ...
    for (auto it = prediction.begin(); it != prediction.end();
            ++it, ++target_it) {
        //Find closest neighbour in k+1 state measurements
        float min_distance = FLT_MAX;
        vect3f closest_pos;
        for (auto it2 = new_positions.begin(); it2 != new_positions.end();
                ++it2) {
        ...
        ublas::matrix<float> k_factor(6, 3, 0), tmp_matrix, tmp_matrix2,
                inversed, meas_matrix;

        //update p factor
        tmp_matrix = ublas::prod(transition_, (*target_it)->get_p_factor());

        (*target_it)->set_p_factor(
                ublas::prod(tmp_matrix, ublas::trans(transition_))
                        + sensors_[(*target_it)->get_sensor_id()]->get_process_covariance());
        ...
        tmp_matrix =
                ublas::prod(tmp_matrix2, tmp_matrix)
                        + sensors_[(*target_it)->get_sensor_id()]->get_measurement_covariance();
        ...
        tmp_matrix = meas_matrix
                - ublas::prod(output_, (*target_it)->get_state());
        tmp_matrix = targets_[it - prediction.begin()]->get_state()
                + ublas::prod(k_factor, tmp_matrix);
        (*target_it)->set_state(tmp_matrix);

        //update p factor
        tmp_matrix = ublas::identity_matrix<float>(6)
                - ublas::prod(k_factor, output_);
        (*target_it)->set_p_factor(
                ublas::prod(tmp_matrix, (*target_it)->get_p_factor()));
    }
}
