...
class target {
public:
    target(int sensor_id, boost::numeric::ublas::matrix<float> state,
            boost::numeric::ublas::matrix<float> p_factor)
            : id_(gId_++), sensor_id_(sensor_id), state_(state), p_factor_(
                    p_factor) {
    }
    /**
