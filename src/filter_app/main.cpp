...
void start_filter(std::shared_ptr<filter_module> filter_module_,
        std::shared_ptr<blocking_queue> b_queue,
        std::shared_ptr<sending_buffer> sending_buffer) {
...
int main(int argc, char ** argv) {
    ...
    if (argc != 4) {
        std::cout
                << "Usage: filter_app <gen_host> <gen_port> <filt_server_port>"
                << std::endl;
    } else {
        ...
        boost::thread filter_main_thread(
                boost::bind(start_filter, filter_module::get_instance(),
                        b_queue, filter_sending_buf));
        client_thread.join();
