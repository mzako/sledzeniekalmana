...
balistic::balistic(commons::vect3f velocity, float air_resistance)
        : velocity_(velocity) {
    if (air_resistance < 0) {
        air_resistance_ = 0;
    } else {
...
vect3f balistic::get_position(float time) const {
    ...
    if (air_resistance_ == 0) {
        ...
        pos.x_ = vxm * log(1 + air_resistance_ * vxm * velocity_.x_ * time)
                / air_resistance_;
        pos.y_ = vym * log(1 + air_resistance_ * vym * velocity_.y_ * time)
                / air_resistance_;
        pos.z_ = velocity_.z_ * time - vzm * GRAVITY_ * time * time / 2;
    }
    return pos;
