...
measurement_dto sensor_observer::make_measurement(
        std::shared_ptr<target> obj) const {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::normal_distribution<> d(0,
            simulation_module::TIME_STEP_ * simulation_module::TIME_STEP_ / 2);
    vect3f pos;
...
std::vector<measurement_dto> sensor_observer::get_positions() const {
    ...
    for (auto element : targets_) {
        positions.push_back(
                measurement_dto(element->get_current_position(),
                        element->get_id()));
    }
    return positions;
}
