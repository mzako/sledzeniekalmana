...
server::server(int port, std::shared_ptr<sending_buffer> ptr,
        ...
        : port_(port), initial_data_(initial_data), sending_buf_(ptr), thread_group_(
                new boost::thread_group), is_started_(true) {
}

void server::send(std::shared_ptr<ip::tcp::socket> socket,
        std::shared_ptr<sending_buffer> sending_buf) {
    std::cout << "Sending thread" << (unsigned int *) this << "is_started_"
            << is_started_ << std::endl;
...
void server::operator()() {
    try {
        ...
        while (is_started_) {
            ...
            if (is_started_) {
                boost::function<
                        void(std::shared_ptr<ip::tcp::socket>,
                                std::shared_ptr<sending_buffer>)> run(
                        boost::bind(&server::send, this, _1, _2));
                thread_group_->create_thread(
                        boost::bind(run, socket, sending_buf_));
            }
        }
...
void server::stop() {
    ...
    std::cout << "Stop" << (unsigned int *) this << "is_started_" << is_started_
            << std::endl;
